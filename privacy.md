...
- Subscription information that Apple and our payments processor (RevenueCat, Inc.) need to operate your in-app purchase.
- Aggregated, pseudonymous usage analytics — which screens you visit, which buttons you tap, and the outcome of those actions — that we collect through Google's Firebase Analytics service so we can understand how the App is used and improve it.
- Crash reports and stability diagnostics, collected through Google's Firebase Crashlytics service, that are sent only when the App terminates abnormally so we can identify and fix the underlying bug.
- Any voluntary information you send us when you contact us for support.
...
- We do not collect the iOS device advertising identifier (IDFA). We have deliberately omitted Firebase's optional "AdSupport" / "AnalyticsIdentitySupport" component, which is the part of Firebase Analytics that would request the IDFA. Because we do not request the IDFA and we do not share any identifier you provide with a data broker, the App does not engage in "tracking" within the meaning of Apple's App Tracking Transparency framework, and we therefore do not present the system tracking prompt.
- We do not run third-party advertising software development kits inside the App.
...
- The optional IDFA-collection component (FirebaseAnalyticsIdentitySupport / AdSupport) is **not** linked, so no advertising identifier is collected;
- Data is **not** used by Google for personalised advertising;
- Default analytics retention is set to the shortest setting we are able to select in the Firebase console (currently 2 months for event-level data).
...
- Reports are sent only after an abnormal termination, never during normal operation;
- No identifier you provide (name, e-mail, payment instrument) is attached to the report;
- The collection toggle exposed in our in-app Settings turns Crashlytics collection on or off at the SDK level, in parallel with Firebase Analytics.

Google's handling of personal data under both services is governed by the [Google Privacy Policy](https://policies.google.com/privacy), the [Firebase Privacy and Security Information](https://firebase.google.com/support/privacy), and the data processing terms applicable to Firebase customers.
...
- **Firebase Analytics event-level data**: retained for the shortest period we are able to configure in the Firebase console (currently 2 months). After that period, the underlying event records are deleted; only aggregated, non-personal statistics survive.
- **Firebase Analytics user-level data**: retained for as long as the corresponding pseudonymous app instance identifier remains active. Reinstalling the App generates a new identifier, after which the previous one expires according to Google's retention rules.
- **Firebase Crashlytics crash reports**: retained according to the Firebase Crashlytics default retention period (90 days for full report payloads), after which the underlying records are deleted and only aggregated, non-personal stability statistics survive.
...
- For transfers to RevenueCat: the **Standard Contractual Clauses** adopted by the European Commission (and the UK International Data Transfer Addendum, where applicable), supplemented by our data processing addendum with RevenueCat.
- For transfers to Google in connection with Firebase Analytics and Firebase Crashlytics: the **Standard Contractual Clauses** included in the Google Ads Data Processing Terms and the Firebase Data Processing and Security Terms, and, for transfers to the United States, the **EU–U.S. Data Privacy Framework** to which Google LLC has certified.
